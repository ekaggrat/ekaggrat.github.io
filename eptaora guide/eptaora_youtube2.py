...
YOUTUBE_TEMPLATE = """
            <div class="snap-item">
                <div class="w-full h-full flex items-center justify-center bg-white p-6">
                    <div class="w-full aspect-[1241/875] max-h-full bg-gray-50 flex justify-center">
                        <iframe 
                            class="w-full h-full" 
...
def generate_gallery():
    ...
    # Parse youtube links and ignore anything after the second comma
    youtube_links = {}
    yt_file_path = os.path.join(IMAGE_FOLDER, 'youtube_links.txt')
    if os.path.exists(yt_file_path):
        with open(yt_file_path, 'r', encoding='utf-8') as yf:
            for line in yf:
                if ',' in line:
                    parts = line.split(',') # Splits the line on ALL commas
                    if len(parts) >= 2:
                        try:
                            # We only care about the first two parts (index 0 and 1)
                            img_num = int(parts[0].strip())
                            video_id = extract_youtube_id(parts[1].strip())
                            youtube_links[img_num] = video_id
                        except ValueError:
                            print(f"Skipping invalid youtube link line: {line.strip()}")

    # Find project description (ignoring myrole.txt and youtube_links.txt)
    txt_files = [f for f in os.listdir(IMAGE_FOLDER) if f.lower().endswith('.txt') and f.lower() not in ['myrole.txt', 'youtube_links.txt']]
    ...
    for index, filename in enumerate(files):
        ...
        # Check if the current 1-based image index has an associated YouTube video
        current_image_number = index + 1
        if current_image_number in youtube_links:
            video_html = YOUTUBE_TEMPLATE.replace("VIDEO_ID", youtube_links[current_image_number])
            gallery_items += video_html
